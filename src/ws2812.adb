...
package body WS2812 is
   ...
   Pin_Config : constant MSPM0.GPIO.PINCM_Register :=
      (WCOMP      => False,
       WUEN       => False,
       INV        => False,
       HIZ1       => False,
   ...
   type Bit_Index is mod 24;
   type Compare_Array is array (LED_Index, Bit_Index) of UInt8
   ...
   begin
      Data := Shift_Left (UInt32 (Color.G), 16) or
              ...
              UInt32 (Color.B);
      for I in Bit_Index'Range loop
         if (Shift_Right (Data, 23 - Natural (I)) and 1) = 0 then
            CCVAL (LED, I) := CCVAL_0;
         else
            CCVAL (LED, I) := CCVAL_1;
         end if;
   ...
   procedure Update is
      DMA_CTL : constant MSPM0.DMA.DMACTL_Register :=
